$ontext
This is a simple Ecenomic Dispatch problem with load variation.
There are 5 gnerators
'a','b','c' are quadratic cost coefficient
Pmin  Pmax are generator limit.
10 different scenarios are simulated
$offtext

set gen /g1*g5/
    counter /c1*c10/;

Parameter loading(counter);
loading(counter)  =  0.6+ord(counter)/10;
*The previous line creates a vector=[0.7,0.8.... up to 1.7]

parameter solution(counter,*);

scalar BaseDemand /400/;
scalar demand;
table gendata(gen,*)
...
*Start of looping
loop(counter,
demand= BaseDemand*loading(counter);
solve EcenomicDispatch using qcp minimizing OF;
solution(counter,'load')=demand;
solution(counter,'OF')=OF.l;
solution(counter,gen)=p.l(gen);
);
*end of loop
